...
export const SummaryScreen: React.FC = () => {
  ...
  const handleBatchGenerate = async () => {
    ...
    try {
      ...
      if (pendingTasks.length > 0) {
        ...
        Alert.alert(
          t('common.info', '提示'),
          t('summary.all_processing', '所有检测到的遗失项均已在处理中')
        )
      }
    } catch (e) {
  ...
  const handleDeleteSummary = async (id: string) => {
    ...
    const title =
      summary.type === 'weekly'
        ? t('summary.week_num', `第 ${getWeekNumber(new Date(summary.startDate))} 周`, {
            week: getWeekNumber(new Date(summary.startDate))
          })
        : summary.type === 'monthly'
          ? t('summary.month_num', `${new Date(summary.startDate).getMonth() + 1}月`, {
              month: new Date(summary.startDate).getMonth() + 1
            })
          : summary.type === 'quarterly'
            ? t(
                'summary.quarter_num',
                `${new Date(summary.startDate).getFullYear()}年Q${Math.ceil((new Date(summary.startDate).getMonth() + 1) / 3)}`,
                {
                  year: new Date(summary.startDate).getFullYear(),
                  quarter: Math.ceil((new Date(summary.startDate).getMonth() + 1) / 3)
                }
              )
            : t('summary.year_num', `${new Date(summary.startDate).getFullYear()}年`, {
                year: new Date(summary.startDate).getFullYear()
              })

    Alert.alert(
      t('common.confirm_delete', '确认删除'),
      t('summary.delete_confirm_title', `确定要删除「${title}」的总结吗？此操作不可撤销。`, {
        title
      }),
      [
        ...
        <View style={[styles.container, { backgroundColor: colors.bgApp }]}>
          ...
          >
            <View style={styles.headerTopLine}>
              <View>
                <Text style={[styles.superTitle, { color: colors.textPrimary }]}>
                  {t('summary.title', '算力演算场')}
                </Text>
          ...
          {activeTab === 'panel' ? (
            ...
            >
              ...
              {(missingSummaries.length > 0 || stats.totalDiaryCount > 0) && (
                <View
                  style={[styles.aiSuggestionsContainer, { backgroundColor: colors.bgSurface }]}
                >
                  <View style={styles.aiSuggestionsHeader}>
                    <Text style={[styles.aiSuggestionsTitle, { color: colors.textPrimary }]}>
                    ...
                    <View style={styles.aiSuggestionsActions}>
                      ...
                      >
                        <Text
                          style={[styles.concurrencyButtonText, { color: colors.textSecondary }]}
                        >
                      ...
                      {[1, 2, 3, 4, 5].map((n) => (
                        <TouchableOpacity
                          ...
                          style={[
                            ...
                            {
                              backgroundColor:
                                n === concurrencyLimit ? colors.primary + '20' : 'transparent'
                            }
                          ]}
                          onPress={() => {
                        ...
                        >
                          <Text
                            style={[
                              ...
                              {
                                color: n === concurrencyLimit ? colors.primary : colors.textPrimary
                              }
                            ]}
                          >
                  ...
                  {missingSummaries.length === 0 ? (
                    <Text style={[styles.noMissingText, { color: colors.textSecondary }]}>
                    ...
                    <View style={styles.missingList}>
                      {missingSummaries.map((mp, index) => {
                        const uKey = `${mp.type}_${new Date(mp.startDate).getTime()}`
                        const isGen =
                          !!generationStates[uKey] && generationStates[uKey].status !== 'error'
                        ...
                        return (
                          ...
                          >
                            ...
                            <View style={styles.missingItemInfo}>
                              <Text
                                style={[styles.missingItemTitle, { color: colors.textPrimary }]}
                              >
                                {mp.label || mp.dateRangeStr}
                              </Text>
                              <Text
                                style={[styles.missingItemDate, { color: colors.textSecondary }]}
                              >
                                {mp.startDate && new Date(mp.startDate).toLocaleDateString()} -{' '}
                                {mp.endDate && new Date(mp.endDate).toLocaleDateString()}
                              </Text>
                              <Text style={[styles.missingItemBadge, { color: colors.primary }]}>
                            ...
                            >
                              <Text
                                style={[styles.generateButtonText, { color: colors.textOnPrimary }]}
                              >
                                {isGen ? (progress >= 100 ? '✓' : '⏳') : '✨'}
                              </Text>
                            </TouchableOpacity>
