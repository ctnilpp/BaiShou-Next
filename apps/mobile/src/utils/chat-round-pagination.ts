/** 每页（隐藏分页）包含的对话轮数 */
export const CHAT_ROUNDS_PER_PAGE = 5
...
/** 按 id 去重，保留首次出现（时间正序列表中取靠前项） */
export function dedupeMessagesById<T extends ChatLikeMessage>(messages: readonly T[]): T[] {
...
export function applyCacheToWindow<T extends ChatLikeMessage>(
  cache: readonly T[],
  roundWindowStart: number,
  fetchHasMore: boolean
): { display: T[]; hasMore: boolean; roundWindowStart: number } {
  const uniqueCache = dedupeMessagesById(cache)
  const rounds = groupMessagesIntoRounds(uniqueCache)
  const clampedStart = Math.min(roundWindowStart, computeInitialRoundWindowStart(rounds.length))
  const display = dedupeMessagesById(flattenRoundSlice(rounds, clampedStart))
  return {
    display,
