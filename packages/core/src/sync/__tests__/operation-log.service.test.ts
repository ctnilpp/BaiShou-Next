...
const makeLog = (
  overrides: Partial<SyncSessionLog> = {},
): SyncSessionLog => ({
...
describe('OperationLogService', () => {
  ...
  describe('writeLog', () => {
    ...
    it('should create log directory if it does not exist', async () => {
      ...
      // cleanup nested dir
      fs.rmSync(path.dirname(path.dirname(nestedDir)), { recursive: true, force: true });
    });
  });
  ...
  describe('getLastSyncSummary', () => {
    it('should return summary of the most recent log', async () => {
      const expectedSummary: SyncSummary = { uploaded: 5, downloaded: 2, deletedRemote: 1, deletedLocal: 0, conflicts: 1, skipped: 10 };
      await service.writeLog(makeLog({ sessionId: 'older', completedAt: '2026-05-16T00:00:00.000Z', summary: { uploaded: 9, downloaded: 0, deletedRemote: 0, deletedLocal: 0, conflicts: 0, skipped: 0 } }));
      await service.writeLog(makeLog({ sessionId: 'newer', completedAt: '2026-05-17T00:00:00.000Z', summary: expectedSummary }));
  ...
  describe('cleanupOldLogs', () => {
    it('should keep only the specified number of most recent logs', async () => {
      for (let i = 0; i < 10; i++) {
        await service.writeLog(makeLog({ sessionId: `log-${i}`, completedAt: `2026-05-${(10 + i).toString().padStart(2, '0')}T00:00:00.000Z` }));
      }
