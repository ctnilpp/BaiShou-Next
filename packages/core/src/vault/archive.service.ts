...
export class DataArchiveService {
  ...
  async listSnapshots(maxCount = 5): Promise<string[]> {
    ...
    const stats = await Promise.all(
      zips.map(async (f) => ({ path: f, mtime: (await fs.stat(f)).mtimeMs }))
    )
  ...
  async exportToTempFile(): Promise<string | null> {
    ...
    return new Promise(async (resolve, reject) => {
      output.on('close', () => {
        console.info(
          `[DataArchiveService] Exported archive stream closed (${archive.pointer()} bytes).`
        )
        resolve(finalPath)
      })
      ...
      const exploreDir = (currentPath: string, relativePath: string) => {
        ...
        for (const entry of entries) {
          ...
          if (statSync(fullPath).isDirectory()) {
            ...
            if (entry === 'snapshots' || entry === 'temp' || entry.startsWith('BaiShou_Archive'))
              continue
            exploreDir(fullPath, zipRelative)
          } else {
            // 排查 SQLite 的死锁事务缓存
            if (
              zipRelative.endsWith('-wal') ||
              zipRelative.endsWith('-shm') ||
              zipRelative.endsWith('-journal')
            ) {
  ...
  async importFromZip(
    zipPath: string
  ): Promise<{ deviceConfig?: DevicePreferences; filesRestored: boolean }> {
    ...
    try {
      ...
      const globalRegistryPath = path.join(
        await this.pathProvider.getGlobalRegistryDirectory(),
        'vault_registry.json'
      )
