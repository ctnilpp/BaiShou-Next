...
export class ModelPricingService {
  ...
  public async calculateCostMicros(
    ...
    if (!price) {
      logger.info(
        `[ModelPricingService] No price found for ${providerId}/${modelId}. Defaulting to 0.`
      )
      return 0
    }
    ...
    logger.info(
      `[ModelPricingService] Calculation for ${providerId}/${modelId}: inputTokens=${usage.inputTokens}, outputTokens=${usage.outputTokens} -> USD=${costInUSD}`
    )
